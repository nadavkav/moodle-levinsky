...
ns.LibrarySelector.prototype.loadSemantics = function (library, params) {
  ...
  // Editor UI RTL support.
  var directionality = '';
  if (H5PIntegration.editor.editordirsupport === '1') {
    if (that.defaultParams.language === null || that.defaultParams.language === undefined) {
        // Add h5peditor-dir-rtl|ltr to h5peditor div, based on iframe's parent document directionality.
        // Infer default directionality from hosing application.
        var directionality = 'h5peditor-dir-' + window.parent.document.dir;
    } else {
        // Library's predefined content language override the default hosting framework directionality.
        if (that.defaultParams.language == 'he' || that.defaultParams.language == 'ar') {
            directionality = 'h5peditor-dir-rtl';
        } else {
            // TODO: Do we need this?
            //directionality = 'h5peditor-dir-ltr';
        }
    }
  }
  if (library === '-') {
