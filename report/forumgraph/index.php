...
$linktoforum = new moodle_url($CFG->wwwroot.'/mod/forum/view.php', array('f'=> $cm->instance));
...
$schoolmenu = html_writer::select($schooloptions, "school", $school, get_string('choose', 'report_forumgraph'),
    array('onchange'=>'loadCourseMenu(this.options[this.selectedIndex].value);'));
...
if ($course || (!$course && $school)) {
    ...
    if (empty($coursenames)) {
        $courses[] = $course;
        $coursenames[$course] = $COURSE->fullname;
    }
    report_forumgraph_get_category_courses($school, $courses, $coursenames);
    $coursemenu = html_writer::select($coursenames, 'course', $course, get_string('choose', 'report_forumgraph'),
        array('onchangez'=>'loadForumMenu(this.options[this.selectedIndex].value)'));
} else {
    $coursemenu = html_writer::select(array(), 'course', $course, get_string('choose', 'report_forumgraph'),
        array('onchange'=>'loadForumMenu(this.options[this.selectedIndex].value)'));
}
...
echo html_writer::link($linktoforum, get_string('backtoforum', 'report_forumgraph'), array('class'=>'btn btn-default pull-left'));
...
echo $OUTPUT->heading(html_writer::link($linktoforum, $heading));
...
if (!empty($school) && !empty($course) && !empty($forum)) {
    ...
    if ($discussions) {
        ...
        $limit = 3;
        $mpus = $DB->get_records_sql("SELECT userid, COUNT(fp.userid) AS postcount 
          FROM {forum_posts} fp WHERE discussion $in_sql GROUP BY fp.userid ORDER BY postcount DESC LIMIT $limit", $in_params);
        ...
        if ($mpus) {
            ...
            $samenumpostuser = $DB->get_records_sql("SELECT userid, COUNT(fp.userid) AS postcount 
              FROM {forum_posts} fp WHERE discussion $in_sql GROUP BY fp.userid HAVING postcount = ".$lastuser->postcount, $in_params);
    ...
    echo $OUTPUT->heading(get_string('forumgraph', 'report_forumgraph'), 3);
    ...
    echo html_writer::link($linktoforum, get_string('backtoforum', 'report_forumgraph'), array('class'=>'btn btn-default'));
}
