 ...
 * @param stdClass $cm
 */
function report_forumgraph_extend_navigation_module($navigation, $cm) {
    if ($cm->modname == 'forum' && has_capability('report/forumgraph:view', context_course::instance($cm->course))) {
        $url = new moodle_url('/report/forumgraph/index.php', array('course' => $cm->course, 'forum' => $cm->instance));
        //http://moodle2.levinsky.ac.il/report/forumgraph/index.php?school=6&course=4034&forum=6087
        $navigation->add(get_string('pluginname', 'report_forumgraph'), $url, navigation_node::TYPE_SETTING, null, null, new pix_icon('i/report', ''));
    }
}